...
describe('generateResumeJson', () => {
  ...
  it('adds JavaScript/TypeScript to the Programming category when missing', () => {
    const result = generateResumeJson(createExistingResumeJson());
    const programming = result.skills.find((category) => category.category === 'Programming');

    expect(programming?.items).toContain(TARGET_SKILL);
    expect(programming?.items.filter((item) => item === TARGET_SKILL)).toHaveLength(1);
  });
...
describe('readExistingResumeJson', () => {
  ...
  it('logs a descriptive error and exits when the JSON file is missing', () => {
    ...
    expect(() => readExistingResumeJson('/does/not/exist.json')).toThrow('process.exit:1');
    expect(errorSpy).toHaveBeenCalledWith(
      expect.stringContaining('❌ Failed to read or parse /does/not/exist.json:'),
      expect.any(Error)
    );
...
describe('writeResumeJson', () => {
  ...
  it('writes the generated JSON with a trailing newline', () => {
    ...
    try {
      ...
      const written = readExistingResumeJson(filePath);
  ...
  it('logs a descriptive error and exits when writing fails', () => {
    ...
    try {
      expect(() => writeResumeJson(createExistingResumeJson(), filePath)).toThrow('process.exit:1');
      expect(errorSpy).toHaveBeenCalledWith(
