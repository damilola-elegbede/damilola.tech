...
export async function GET(request: Request) {
  const ctx = createRequestContext(request, '/api/admin/usage');

  return withRequestContext(ctx, async () => {
    logColdStartIfNeeded();
    logger.request.received('/api/admin/usage', { method: 'GET' });
    ...
    try {
      ...
      logger.request.completed('/api/admin/usage', ctx.startTime, {
      ...
      logger.request.failed('/api/admin/usage', ctx.startTime, error);
