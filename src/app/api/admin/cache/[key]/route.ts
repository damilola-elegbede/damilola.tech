...
export async function GET(
  ...
  const ctx = createRequestContext(request, '/api/admin/cache/[key]');

  return withRequestContext(ctx, async () => {
    logColdStartIfNeeded();
    logger.request.received('/api/admin/cache/[key]', { method: 'GET' });
    ...
    try {
      ...
      logger.request.completed('/api/admin/cache/[key]', ctx.startTime, { key, hit: true });
      return Response.json(cached);
    } catch (error) {
      logger.request.failed('/api/admin/cache/[key]', ctx.startTime, error, { key });
...
export async function PUT(
  ...
  return withRequestContext(ctx, async () => {
    logColdStartIfNeeded();
    logger.request.received('/api/admin/cache/[key]', { method: 'PUT' });
    ...
    try {
      ...
      logger.request.completed('/api/admin/cache/[key]', ctx.startTime, { key });
