 ...
 */
async function validateUrlForSsrf(urlString: string, ip: string): Promise<string | null> {
  let url: URL;
  try {
  ...
  try {
    ...
    const replacements: Record<string, string> = {
      ...
      '{{LEADERSHIP_PHILOSOPHY}}':
        content.leadershipPhilosophy || '*Leadership philosophy not available.*',
    ...
    logger.error('prompt.fetch_failed', {
      error: error instanceof Error ? error.message : String(error),
    });
...
export async function POST(req: Request) {
  const ctx = createRequestContext(req, '/api/resume-generator');

  return withRequestContext(ctx, async () => {
    logColdStartIfNeeded();
    logger.request.received('/api/resume-generator', { method: 'POST' });
    ...
    try {
      ...
      const rateLimitResult = await checkGenericRateLimit(RATE_LIMIT_CONFIGS.resumeGenerator, ip);
      if (rateLimitResult.limited) {
        logger.security.rateLimitTriggered(ip, '/api/resume-generator', {
      ...
      if (isUrl(jobDescription)) {
        ...
        const ssrfError = await validateUrlForSsrf(extractedUrl, ip);
        if (ssrfError) {
      ...
      logger.debug('ats.calculating_score');
      ...
      logger.info('ats.score_calculated', { score: atsScore.total });
      ...
      updateRequestContext({ sessionId: resumeSessionId });

      logger.stream.started('/api/resume-generator');
            ...
            try {
              ...
              for await (const event of stream) {
                if (event.type === 'content_block_delta' && event.delta.type === 'text_delta') {
                  // Log time to first byte (after metadata)
                  if (!firstByteLogged) {
                ...
                checkTimeoutWarning(ctx.startTime, '/api/resume-generator');
              }
              controller.close();
      ...
      logger.request.failed('/api/resume-generator', ctx.startTime, error);
